<#
.SYNOPSIS
    Statically validates a compiled plugin against the game's assemblies.

.DESCRIPTION
    Loads the built DLL with Mono.Cecil, resolves every referenced type, method and
    field against the Valheim managed assemblies / BepInEx core assemblies, and
    reports anything that does not resolve. This catches typos and API drift in
    signatures without having to launch the game.
...
if (-not $Assembly) { $Assembly = Join-Path $root 'dist\ValheimPingHud.dll' }
...
function Check-Type([Mono.Cecil.TypeReference]$type) {
    ...
    if ($type.Namespace -and $type.Namespace.StartsWith('ValheimPingHud')) { return }
